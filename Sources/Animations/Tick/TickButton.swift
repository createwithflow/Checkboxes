...
@IBDesignable
class TickButton: ToggleButton {
    private lazy var tick: TickView = {
        let startView = TickView(frame: .zero)
    ...
    override var forward: Timeline? {
        return TickTimeline(view: tick, duration: 0.33)
    }

    override var reverse: Timeline? {
        return TickTimeline(view: tick, duration: 0.33).reversed
    }
