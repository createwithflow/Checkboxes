...
@IBDesignable
class OvertButton: ToggleButton {
    private lazy var overt: OvertView = {
        let startView = OvertView(frame: .zero)
    ...
    override var forward: Timeline? {
        return OvertTimeline(view: overt, duration: 0.5)
    }

    override var reverse: Timeline? {
        return OvertTimeline(view: overt, duration: 0.5).reversed
    }
