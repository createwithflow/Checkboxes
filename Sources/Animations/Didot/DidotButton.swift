...
@IBDesignable
class DidotButton: ToggleButton {
    private lazy var didot: DidotView = {
        let startView = DidotView(frame: .zero)
    ...
    override var forward: Timeline? {
        return DidotTimeline(view: didot, duration: 0.5)
    }

    override var reverse: Timeline? {
        return DidotTimeline(view: didot, duration: 0.5).reversed
    }
