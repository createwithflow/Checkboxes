...
@IBDesignable
class MondoButton: ToggleButton {
    private lazy var mondo: MondoView = {
        let startView = MondoView(frame: .zero)
    ...
    override var forward: Timeline? {
        return MondoTimeline(view: mondo, duration: 0.5)
    }

    override var reverse: Timeline? {
        return MondoTimeline(view: mondo, duration: 0.5).reversed
    }
